...
(function() {
  var Action, fireByResult, makeNodeCb, makeQueryStrR, sequenceStep, spawn,
  ...
  Action.fuseSignal = function(actions, fireAtError) {
    ...
    if (l > 0) {
      return new Action(function(cb) {
        var action, fireByIndex, flags, i, j, len, m, ref, results, returns;
        results = new Array(l);
        returns = new Array(l);
        flags = new Array(l);
        ...
        fireByIndex = function(index) {
          return function(data) {
            if ((data instanceof Error) && fireAtError) {
              flags[index] = false;
              return cb(data);
            } else {
              results[index] = data;
              flags[index] = true;
              if (indexOf.call(flags, false) < 0) {
                return cb(results);
              }
            }
          };
        };
        for (i = m = 0, len = actions.length; m < len; i = ++m) {
          action = actions[i];
  ...
  sequenceStep = function(countUp, limit, stopAtError, actions, results, cb) {
    return actions[countUp]._go(function(data) {
      if ((data instanceof Error) && stopAtError) {
        return cb(data);
      } else {
        ...
        if (countUp === limit) {
          ...
          return sequenceStep(countUp, limit, stopAtError, actions, results, cb);
        }
      }
    });
  };
  ...
  Action.sequence = function(actions, stopAtError) {
    ...
    if (l > 0) {
      return new Action(function(cb) {
        var results;
        results = new Array(l);
        return sequenceStep(0, l, stopAtError, actions, results, cb);
      });
    } else {
      return Action.wrap([]);
    }
